...
tfts includes a migration script that handles both core and provider imports:
...
# or: npx tsx migrate-from-cdktf.ts .
```

The script automatically:
- Changes `from "cdktf"` → `from "tfts"`
- Converts `@cdktf/provider-*/lib/provider` → `./.gen/providers/hashicorp/*/provider.js`
- Converts `@cdktf/provider-*/lib/resource-name` → `./.gen/providers/hashicorp/*/resources/resource-name.js`
- Converts `@cdktf/provider-*/lib/data-*` → `./.gen/providers/hashicorp/*/data-sources/*.js`

### Using ast-grep

For simpler migrations (core imports only):
